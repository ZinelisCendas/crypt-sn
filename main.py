...
import asyncio, json, os, math, signal, time, statistics, logging, functools
from typing import List, Dict, Sequence
from dataclasses import dataclass, field
import aiohttp, websockets, pandas as pd
...
class Trade: ts:int; side:str; token:str; qty:float; price:float; pnl:float
@dataclass(slots=True)
class WalletMetrics:
    address:str; sharpe:float; realised:float; win_rate:float; trades:int
    def is_strong(self)->bool: return self.sharpe>1.2 and self.win_rate*100>=55 and self.realised>0
@dataclass(slots=True)
class Position:
    token:str; qty:float; entry:float; value:float; sl:float; tp:float; src:str

# ---------------- gmgn + solscan + pyth -------------------
class GmgnAPI:  # minimal wrapper
    def __init__(self): self.g=gmgn(); self.http=aiohttp.ClientSession()
...
async def pyth_atr(mint:str, minutes:int=ATR_LOOKBACK_MIN)->float:
    ...
    async with aiohttp.ClientSession() as s:
        async with s.get(url) as r:
            if r.status!=200: return 0.05  # fallback vol
            data=await r.json(); prices=[p[1] for p in data.get("prices", [])][-minutes:]
    if len(prices)<2: return 0.05
...
class WalletAnalyzer:
    def __init__(self,tf="30d"): self.tf=tf; self.api=GmgnAPI()
    async def _one(self,addr):
        info, trades=await asyncio.gather(self.api.info(addr,self.tf), self.api.trades(addr,self.tf))
        if not info or info.get("code")!=0: return None
        pnl_by_day={}
        wins=0
        for t in trades:
            day=time.strftime('%Y-%m-%d', time.gmtime(int(t['timestamp'])/1000))
            pnl_by_day[day]=pnl_by_day.get(day,0)+float(t['pnl'])
            if float(t['pnl'])>0: wins+=1
        if len(pnl_by_day)<2: sharpe=0
        else:
            series=pd.Series(list(pnl_by_day.values()))
            sharpe=series.mean()/max(series.std(),1e-6)
        win_rate=wins/max(1,len(trades))
        return WalletMetrics(info['data']['address'], sharpe, float(info['data']['totalRealizedProfit']), win_rate, len(trades))
...
# ------------------- execution layer ----------------------
class JupiterExec:
    def __init__(self): self.http=aiohttp.ClientSession()
    async def quote(self,mint_in,mint_out,amount):
        ...
        async with self.http.get(url) as r: r.raise_for_status(); return await r.json()
    async def swap_tx(self,route):
        async with self.http.post(f"{JUPITER_URL}/v6/swap",json={"route":route,"userPublicKey":route['inAmountAddress']}) as r:
            r.raise_for_status(); tx=await r.json(); return tx['swapTransaction']
...
class PositionBook:
    ...
    def nav(self):
        ...
        return free+pos
    def update_peak(self,nav): self.peak=max(self.peak,nav)
    def global_dd(self,nav): return 100*(1-nav/self.peak)
...
class Notifier:
    ...
    async def send(self,msg):
        ...
        await aiohttp.ClientSession().post(self.api,json={"chat_id":TELE_CHAT,"text":msg})
...
class CopyEngine:
    ...
    async def _execute_buy(self,ev):
        token=ev['token']; price=float(ev['price']); src=ev['address']
        nav=self.pb.nav()
